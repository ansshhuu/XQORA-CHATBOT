...
Ask/decline state ("asked" / "declined" / "given") is owned by
app.services.session_service now - see SESSION_STATE.md. Once a session has
been asked, it's asked at most once: given feedback, declined, or ignored
all latch the state (persisted, so it survives a restart) so the prompt
never repeats and never blocks normal conversation afterward.
"""
import re
...
def should_ask(session_id: str) -> bool:
    ...
    return session_service.should_ask_feedback(session_id)


def mark_asked(session_id: str, db: Session | None = None) -> None:
    session_service.set_feedback_status(session_id, "asked", db=db)


def is_awaiting_response(session_id: str) -> bool:
    return session_service.is_awaiting_feedback_response(session_id)


def reset(session_id: str) -> None:
    session_service.reset_feedback_status(session_id)


def mark_declined(session_id: str, db: Session | None = None) -> None:
    ...
    session_service.set_feedback_status(session_id, "declined", db=db)


def _mark_given(session_id: str, db: Session | None = None) -> None:
    session_service.set_feedback_status(session_id, "given", db=db)
...
def _handle_feedback_response_locked(db: Session, session_id: str, message: str) -> FeedbackStepResult:
    ...
    if rating is None and comments is None:
        mark_declined(session_id, db=db)
        return FeedbackStepResult(reply="", handled=False)
